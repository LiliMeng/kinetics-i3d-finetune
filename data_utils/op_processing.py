...
tf.app.flags.DEFINE_integer('image_size', 56,
...
def distort_image(image, height, width, bbox, thread_id=0, scope=None):
  ...
  with tf.name_scope(
    ...
    # the third dimension.
    distorted_image.set_shape([height, width, 45])
...
def image_preprocessing(image_buffer, bbox, train, thread_id=0):
  ...
  image = []
  for i in range(15):
    img = decode_jpeg(image_buffer[i])
    image.append(img)
...
def parse_example_proto(example_serialized):
  ...
  feature_map = {
      ...
      'image/encoded9': tf.FixedLenFeature(
          [], dtype=tf.string, default_value=''),
      'image/encoded10': tf.FixedLenFeature(
          [], dtype=tf.string, default_value=''),
      'image/encoded11': tf.FixedLenFeature(
          [], dtype=tf.string, default_value=''),
      'image/encoded12': tf.FixedLenFeature(
          [], dtype=tf.string, default_value=''),
      'image/encoded13': tf.FixedLenFeature(
          [], dtype=tf.string, default_value=''),
      'image/encoded14': tf.FixedLenFeature(
          [], dtype=tf.string, default_value=''),
      'image/encoded15': tf.FixedLenFeature(
          [], dtype=tf.string, default_value=''),
  ...
  features_list=[]
  for i in range(1, 16):
     img_name='image/encoded'+str(i)
     features_list.append(features[img_name])

  filename = tf.cast(features['image/filename'], dtype=tf.string)

  return features_list, label, bbox, features['image/class/text']
...
def batch_inputs(dataset,
  ...
  with tf.name_scope('batch_processing'):
    ...
    for i in range(15):
      tmp_frame= images[:, :, :, 3*i: 3*(i+1)]
      frames_list.append(tmp_frame)

    final_img=tf.stack(frames_list, 1)
    print("final_img********************************")
    print(final_img)
    ...
    label_index_batch -= 0  # 1-based to 0-based.
    
    final_labels = tf.reshape(label_index_batch, [batch_size])
    print("final_labels^^^^^^^^^^^^^^^^^^^^^^^^^^")
    print(final_labels)
